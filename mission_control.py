...
def check_password():
    ...
    if "password_correct" not in st.session_state:
        ...
        st.markdown('<div class="login-box"><div class="rotex-logo-container"><div class="rotex-text">ROTex</div><div class="rotex-tagline">System v30.1</div></div></div>', unsafe_allow_html=True)
...
def load_market_data():
    ...
    try:
        ...
        # 3. HANDLE YFINANCE MULTI-INDEX BUG
        if isinstance(data.columns, pd.MultiIndex):
            if 'Close' in data.columns.get_level_values(0):
                data = data.xs('Close', level=0, axis=1)
            else:
                return df_safe # Weird structure, use fallback

        # 4. HANDLE 'Close' COLUMN VARIATIONS
        if 'Close' in data:
            data = data['Close']
